...
describe("[Challenge] Puppet v2", function () {
    ...
    it("Execution", async function () {
        ...
        const amountNeeded = await lendingPool.calculateDepositOfWETHRequired(POOL_INITIAL_TOKEN_BALANCE);

        await weth.connect(player).deposit({ value: amountNeeded.sub(await weth.balanceOf(player.address)) });
        await weth.connect(player).approve(lendingPool.address, amountNeeded);
